...
enum { 
  QLSize  = sizeof(struct qlinks),
  SizeQ  = ((SizeQuant > QLSize) ? SizeQuant : QLSize),
/* End sentinel: value placed in bsize field of dummy block delimiting
   end of pool block.  The most negative number which will  fit  in  a
   bufsize, defined in a way that the compiler will accept. */
  ESent  = -(((1L << (sizeof(bufsize) * 8 - 2)) - 1) * 2) - 2,
  SizeQuant_1 = SizeQuant-1,
  notSizeQuant_1 = ~SizeQuant_1
};
...
void* bget( bufsize requested_size)
{
...
#ifdef SizeQuant
#if SizeQuant > 1
    size = (size + SizeQuant_1) & notSizeQuant_1;
#endif
#endif
  ...
  while (b != &freelist) {
    ...
    if ((b->bh.bsize - size) > (SizeQ + (sizeof(struct bhead)))) {
        ...
        bn = BH(((char *) ba) + size);
        ES_ASSERT(bn->prevfree == b->bh.bsize);
        ...
        ES_ASSERT(ba->prevfree == b->bh.bsize);

                    /* The buffer isn't big enough to split.  Give  the  whole
           shebang to the caller and remove it from the free list. */

        ES_ASSERT(b->ql.blink->ql.flink == b);
        ES_ASSERT(b->ql.flink->ql.blink == b);
...
void* bgetr( void *buf, bufsize size)
{
  ...
  osize -= sizeof(struct bhead);
    ES_ASSERT(osize > 0);
    memcpy((char *) nbuf, (char *) buf, /* Copy the data */
...
void brel(void *buf)
{
  ...
  ES_ASSERT(b->bh.prevfree == 0);
#ifdef BufStats
  totalloc -= bdh->tsize;
  ES_ASSERT(totalloc >= 0);
  numdrel++;          /* Number of direct releases */
#endif /* BufStats */
#ifdef FreeWipe
  memset((char *) buf, 0x55,
     (MemSize) (bdh->tsize - sizeof(struct bdhead)));
#endif /* FreeWipe */
  ES_ASSERT(relfcn != NULL);
  ...
  bn = NULL;
    ...
    ES_ASSERT(b->bh.bsize < 0);

    /*  Back pointer in next buffer must be zero, indicating the
  same thing: */

    ES_ASSERT(BH((char *) b - b->bh.bsize)->prevfree == 0);
  ...
  ES_ASSERT(BH((char *) b - b->bh.prevfree)->bsize == b->bh.prevfree);
  ...
  ES_ASSERT(freelist.ql.blink->ql.flink == &freelist);
  ES_ASSERT(freelist.ql.flink->ql.blink == &freelist);
  b->ql.flink = &freelist;
  ...
  ES_ASSERT(BH((char *) bn + bn->bh.bsize)->prevfree == bn->bh.bsize);
  ES_ASSERT(bn->ql.blink->ql.flink == bn);
  ES_ASSERT(bn->ql.flink->ql.blink == bn);
  ...
  ES_ASSERT(b->bh.prevfree == 0);
  ES_ASSERT(BH((char *) b + b->bh.bsize)->bsize == ESent);
  ES_ASSERT(BH((char *) b + b->bh.bsize)->prevfree == b->bh.bsize);
  ...
  ES_ASSERT(numpblk == numpget - numprel);
#endif /* BufStats */
    }
#endif /* BECtl */
}
...
void bpool( void *buf, bufsize len)
{
  ...
  pool_len = -1;
    ...
    ES_ASSERT(len - sizeof(struct bhead) <= -((bufsize) ESent + 1));
...
int main()
{
  ...
  ES_ASSERT(bs <= (((bufsize) 4) * ExpIncr));
  bs = blimit(bs);
  if (rand() & 0x400) {
      cb = (char *) bgetz(bs);
  } else {
