...
public:
  ...
  /// Templated services, implemented inplace at compile time
  ///
  template <typename DataT>
  size_t read(DataT& data) ESE_NOTHROW { return read( reinterpret_cast<uint8_t*>(&data), sizeof(DataT) ); }

  template <typename DataT>
  size_t write(const DataT& data) ESE_NOTHROW { return write( reinterpret_cast<const uint8_t*>(&data), sizeof(DataT) ); }
};
