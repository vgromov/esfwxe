...
# define ESE_USB_ISR_TRACE_CFG()      ((void)0)
#endif
#ifndef ESE_USB_ISR_TRACE_ON
# define ESE_USB_ISR_TRACE_ON()       ((void)0)
#endif
#ifndef ESE_USB_ISR_TRACE_OFF
# define ESE_USB_ISR_TRACE_OFF()      ((void)0)
#endif
#ifndef ESE_USBCDCRX_TRACE_CFG
# define ESE_USBCDCRX_TRACE_CFG()     ((void)0)
#endif
#ifndef ESE_USBCDCRX_TRACE_ON
# define ESE_USBCDCRX_TRACE_ON()      ((void)0)
#endif
#ifndef ESE_USBCDCRX_TRACE_OFF
# define ESE_USBCDCRX_TRACE_OFF()     ((void)0)
#endif
#ifndef ESE_USBCDCTX_TRACE_CFG
# define ESE_USBCDCTX_TRACE_CFG()     ((void)0)
#endif
#ifndef ESE_USBCDCTX_TRACE_ON
# define ESE_USBCDCTX_TRACE_ON()      ((void)0)
#endif
#ifndef ESE_USBCDCTX_TRACE_OFF
# define ESE_USBCDCTX_TRACE_OFF()     ((void)0)
#endif

#if defined(USE_USBCDC_TRACE)
# define ESE_USBCDC_TRACE             ES_DEBUG_TRACE
#else
# define ESE_USBCDC_TRACE(...)        ((void)0)
#endif
...
ChannelT(
  rxLen < staticIoBlockLen ? 
    staticIoBlockLen : 
    rxLen, 
  txLen < staticIoBlockLen ? 
    staticIoBlockLen : 
    txLen
),
m_txIdle(1, 1),
...
bool EseUsbCdc::isConnected() const ESE_NOTHROW 
{ 
  return ES_BIT_IS_SET(m_flags, flagConnected); 
}
//-----------------------------------------------------------------------------------

const PCD_InitTypeDef& EseUsbCdc::dcbGet() const ESE_NOTHROW 
{ 
  return m_h.Init; 
}
//-----------------------------------------------------------------------------------

USBD_HandleTypeDef& EseUsbCdc::devHandleGet() ESE_NOTHROW 
{ 
  return m_devh; 
}
//-----------------------------------------------------------------------------------
  
bool EseUsbCdc::shouldYield() const ESE_NOTHROW 
{ 
  return m_shouldYield; 
}
//-----------------------------------------------------------------------------------

void EseUsbCdc::resetYieldFlag() ESE_NOTHROW 
{ 
  m_shouldYield = false; 
}
//-----------------------------------------------------------------------------------
...
//
const EseUsbCdc::ChannelT::HandleT& EseUsbCdc::doHandleGet() const ESE_NOTHROW 
{ 
  return m_h; 
}
//-----------------------------------------------------------------------------------

EseUsbCdc::ChannelT::HandleT& EseUsbCdc::doHandleGet() ESE_NOTHROW 
{ 
  return m_h; 
}
//-----------------------------------------------------------------------------------

void EseUsbCdc::txIdleSet() ESE_NOTHROW
{
  m_txActive = false;
  m_txIdle.unlock(); // Give idle state
}
//-----------------------------------------------------------------------------------
...
bool EseUsbCdc::doActivate() ESE_NOTHROW
{
  ...
  HAL_NVIC_SetPriority(USB_HP_IRQn, USB_HP_IRQ_PRIORITY, 0); 
  ...
  HAL_NVIC_ClearPendingIRQ(USB_HP_IRQn);
...
void EseUsbCdc::onConnectedSet(OnConnectedT proc) ESE_NOTHROW
{
  __disable_irq();
  m_onConnected = proc;
  __enable_irq();
...
{
  if( connected )
    ES_BIT_SET(m_flags, flagConnected);
  else
    ES_BIT_CLR(m_flags, flagConnected);
...
void EseUsbCdc::txIdleSetFromISR() ESE_NOTHROW
{
  m_txActive = false;
...
uint8_t EseUsbCdc::continueTxFromIsr() ESE_NOTHROW
{
  ...
  {
    ESE_USBCDC_TRACE("  prev is %d\n", m_prevBlockLen);

    // Send ZLP if we ended up previous block transfer on % staticIoBlockLen boundary
    if( 
      m_prevBlockLen && 
      0 == (m_prevBlockLen % staticIoBlockLen) 
    )
    {
      m_prevBlockLen = 0;
      USBD_CDC_SetTxBuffer(
    ...
    {
      ESE_USBCDC_TRACE("  completing TX\n");
      
      // set signal TX is idle then
      txIdleSetFromISR();
    }
  }
...
size_t EseUsbCdc::doReceive(esU8* data, size_t toRead, esU32 tmo) ESE_NOTHROW
{
  ...
  ESE_USBCDC_TRACE("  got %d of %d, in tmo=%d\n", pos-data, expected, tmo);
  
  return pos-data;
}
//-----------------------------------------------------------------------------------

rtosStatus EseUsbCdc::txIdleCheck() ESE_NOTHROW
{
  if( m_txActive )
  {
    rtosStatus stat = m_txIdle.lock(1);
    if( rtosOK == stat )
      m_txIdle.unlock(); //< Give semaphore
...
void EseUsbCdc::txActiveSet() ESE_NOTHROW
{
  m_txActive = true;
  m_txIdle.lock(0);
}
...
size_t EseUsbCdc::doSend(const esU8* data, size_t toWrite, esU32 tmo) ESE_NOTHROW
{
  ...
  if( rtosOK == stat )
  {
    ...
    // which may gain access to m_devh at this time
    {
      ...
      m_prevBlockLen = sPos-m_tmpTx;
      USBD_CDC_SetTxBuffer(&m_devh, m_tmpTx, m_prevBlockLen);
...
//-----------------------------------------------------------------------------------
// USB CDC IRQ handlers
//
//-----------------------------------------------------------------------------------

void USB_HP_IRQHandler(void) ESE_NOTHROW
{
