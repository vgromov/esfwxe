...
typedef struct {
  esU32 start;                    // measurement start time, in ms
  esU32 stop;                    // measurement stop time, in ms
  union {
    // weekly schedule
    esU8 weekly;
    // monthly schedule
    ...
    struct {
      esU32 calDays[12];          // each 0-30|29 bit corresponds to the day of month
      esU16 calMonths;            // each 0-11 bit corresponds to the month, 15th bit signals recurrent day event
    };
  };
  esU16 interval;                 // interval between events in minutes
  esU8 mode;                     // schedule mode - interval or calendar
  esU8 dummy;                    // explicit alignment + padding to 64 bytes, which is the 4*n next to 61
