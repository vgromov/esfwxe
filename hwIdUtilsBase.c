...
//--------------------------------------------------------------------------------------------------------------------
//
static inline ESE_STR fmtBuffInc(ESE_STR buff, size_t* buffLen, int len)
{
  if( 0 < len )
  {
    if( (int)(*buffLen) >= len )
...
static inline void fmtBuffTerminate(ESE_STR buff, ESE_STR buffEnd)
{
  if( buffEnd <= buff )
  {
    --buffEnd;
...
#elif defined(ESE_USE_CUSTOM_STRING_DEVICE_INFO) //< ESE_USE_CUSTOM_STRING_DEVICE_INFO should expand into function returning device name string in either long or short form
  if( ES_BIT_IS_SET(flags, APPL_ID_FMT_DESCR_SHORT) )
    len = eseUtilsStrSnprintf(
      buff,
      buffLen,
