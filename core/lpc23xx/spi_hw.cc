...
#define SPI0_ENABLE_PWR   PCONP |= PCSPI
#define SPI0_DISABLE_PWR   PCONP &= ~PCSPI

#define SPI0_CONFIG(pdcb) \
    { esU32 freq = (esU32)SPI0clk / pdcb->freqHz; \
    PCLKSEL0 |= (SPI0div << 16);  \
    SPI0_PINS_CONFIG; \
    S0SPCR = 8;  \
    if( freq < 8 ) freq = 8;  \
...
// Port 0.7 SPI SCK (mode 2),  
// port0.8 MISO (mode 2), 
// port0.9 MOSI (mode 2), SSEL is used in GPIO mode
...
#define SPI1_CONFIG(pdcb) \
  { esU8 div = pdcb->mode == spiMaster ? 2 : 12; \
  esU32 rate = ((esU32)SSP1clk / ((esU32)pdcb->freqHz * (esU32)div)); \
  PCLKSEL1 |= (SSP1div << 20); \
  SPI1_PINS_CONFIG; \
  if( rate > 0 ) --rate;  \
  SPI_PINS_CONFIG1; \
