...
#define UART_BREAK_UNSET(portNum) \
  ...
  #define UART_INIT(portNum, dcb) \
    if( irqInstall( UART ## portNum ##_INT, (void*)&uartISRHandler ## portNum, UART_INTERRUPT_PRIORITY ) == TRUE ) \
    { \
      ports[uart## portNum].dcb = *dcb; \
      ports[uart## portNum].dcr.port = uart## portNum; \
      ports[uart## portNum].dcr.opened = FALSE; \
      ports[uart## portNum].uartTxQueue = xQueueCreate( UART_TX_BUFFLEN, sizeof(esU8) ); \
      ports[uart## portNum].uartRxQueue = xQueueCreate( UART_RX_BUFFLEN, sizeof(esU8) );  \
  ...
  #define UART_INIT(portNum, dcb) \
    if( irqInstall( UART ## portNum ##_INT, (void*)&uartISRWorker ## portNum, UART_INTERRUPT_PRIORITY ) == TRUE ) \
    { \
      ports[uart## portNum].dcb = *dcb; \
      ports[uart## portNum].dcr.port = uart## portNum; \
      ports[uart## portNum].dcr.opened = FALSE; \
      rbInit( (rbHANDLE)&ports[uart## portNum].uartTxQueue, (esU8*)ports[uart## portNum].txData, UART_TX_BUFFLEN  ); \
      rbInit( (rbHANDLE)&ports[uart## portNum].uartRxQueue, (esU8*)ports[uart## portNum].rxData, UART_RX_BUFFLEN ); \
      return (uartHANDLE)&ports[uart## portNum]; \
    }
  ...
  #define UART_IRQ_INIT(portNum) \
    irqInstall( UART ## portNum ##_INT, (void*)&uartISRHandler ## portNum, UART_INTERRUPT_PRIORITY )

#else

  #define UART_IRQ_INIT(portNum) \
    irqInstall( UART ## portNum ##_INT, (void*)&uartISRWorker ## portNum, UART_INTERRUPT_PRIORITY )

#endif

#define UART_IRQ_UNINIT(portNum) \
  UART_DISABLE_INT(portNum); \
  irqUninstall( UART ## portNum ##_INT )
