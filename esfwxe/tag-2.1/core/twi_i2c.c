...
esBL i2cLockPort(i2cHANDLE handle, esU32 timeout)
{
#if defined(USE_FREE_RTOS)
...
esBL i2cIsOpen( i2cHANDLE handle )
{
...
static esBL i2cStart(i2cHANDLE handle)
{
	if( i2cIsOpen(handle) )
	{
		esU32 dwCurRetry = 0;
...
static esU32 i2cMasterIo(i2cHANDLE handle, esU8 addrRW, esU8* buff, esU32 length)
{
	esU32 result = 0;
...
esU32 i2cGetBytes(i2cHANDLE handle, esU8 addr, esU8* pBytes, esU32 count)
{
	if( i2cIsOpen(handle) && pBytes && count )
	{
#ifdef USE_I2C_MASTER_MODE
		if( i2cMaster == CAST_PORT_HANDLE(i2cPortStruct, handle)->dcb.mode )
...
#ifdef USE_I2C_MASTER_MODE
		if( i2cMaster == CAST_PORT_HANDLE(i2cPortStruct, handle)->dcb.mode )
			return i2cMasterIo(handle, addr, (esU8*)pBytes, count);
#endif
#ifdef USE_I2C_SLAVE_MODE
	#error "Not implemented"
#endif
