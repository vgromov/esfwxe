...
void dtExtractTime(const esDT* dt,
	int* hr, int* mins, int* sec,	int* ms)
{
	esDT tmp = (*dt < 0) ? -(*dt) : *dt; // work with positive values
	if( ms )
	...
	int* ms)
{
	...
	*year = dtGetYear(dummy, &dummy); // now dummy contains full year days count
	if( tmp < 0 )
		tmp += ((esI64)dummy) * IMMEDIATE_INT64(86400000); // make the rest of calculations on positive values
	else
		tmp -= ((esI64)dummy) * IMMEDIATE_INT64(86400000);
	...
	int hr, int mins, int sec, int ms)
{
	...
	if( dtIsValid(year, month, dayOfMonth, hr, mins, sec, ms) )
	{
		...
		result += (esI64)dtGetDaysUpToMonth(year, month) + (esI64)(dayOfMonth-1);
		...
		result += ((esI64)fullYearDays) * IMMEDIATE_INT64(86400000);
