...
void schedulerSetRecentEventEnd(esDT evt)
{
...
static schedulerReturnType schedulerScheduleCurrentDate(const esDT* now,
	const CalendarScheduler* schedule, esDT* result)
{
	dtComposeDateTime(&s_time, DT_YEAR0, 1, 1, s_h, s_m, s_s, 0);
	dtComposeDateTime(&s_date, s_nextY, s_nextMn, s_nextDay, 0, 0, 0, 0);
	
	if( s_time >= (esDT)schedule->start && s_time <= (esDT)schedule->stop )
	{
		...
		{
			esBL recurrent = scheduleInterval == schedule->mode ||
			...
			s_time += schedulerMsInMinute * (esDT)schedule->interval; 
			if( s_time <= (esDT)schedule->stop && 
					recurrent )
			{
	...
	else if( s_time < (esDT)schedule->start )
	{
		// use now's date part and start time as a result
		*result = s_date + (esDT)schedule->start;
		return schedulerCurDateScheduled;
	}
...
static void schedulerSearchInit(const esDT* now, esDT* result)
{
...
static void	schedulerScheduleInterval(const CalendarScheduler* schedule, const esDT* now, esDT* result)
{		
			...
			schedulerScheduleCurrentDate(now, schedule, result) )
	{
		...
		*result += (esDT)schedule->start;
	}	
}

static void schedulerScheduleWeekly(const CalendarScheduler* schedule, const esDT* now, esDT* result)
{
	int dow;
	esU8 days;
	schedulerSearchInit(now, result);
...
static void schedulerScheduleMonthly(const CalendarScheduler* schedule, const esDT* now, esDT* result)
{
	esU32 days;
	schedulerSearchInit(now, result);
...
static void schedulerScheduleCalendar(const CalendarScheduler* schedule, const esDT* now, esDT* result)
{
	esU16 month;
	esU32 days;
	schedulerSearchInit(now, result);
...
void schedulerUpdateNextStart(const CalendarScheduler* schedule, const esDT* now, esDT* next)
{
	if( schedule )
	{
...
static esBL schedulerCheckCalendarMonths(const CalendarScheduler* schedule)
{
	esU16 months = SCHEDULER_CALENDAR_MONTH_GET(schedule->calMonths);
	esBL result = months != 0;
	const esU32* days = schedule->calDays; 
...
esBL schedulerIsScheduleValid(const CalendarScheduler* schedule)
{
