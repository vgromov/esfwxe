...
static inline void ekosfUpdateHdrCrc(ProtocolEkosfHdr* hdr)
{
...
static inline void ekosfUpdateHdr(ProtocolEkosfHdr* hdr, esU8 frame, esU16 cmd, esU16 dataLen)
{
  hdr->frameType = frame;
  hdr->command = cmd;
...
static inline void ekosfSetPendingRateChange( EseStdIoState* io, esU32 rate)
{
  // if set rate service is supported by the channel
  // and rate value is not 0
  if( 0 != rate ) // set pending value to rate
    io->pendingRate = rate;
}
...
static inline void ekosfStdIoInit( EseStdIoState* io, EseChannelIo* chnl )
{
...
esU8 ekosfSlaveEnumerate( EseStdIoState* io, esU8 startAddr, esU8 endAddr, ekosfPfnOnEnumDevice onEnum, void* data )
{
  ...
  if( io && io->chnl )
  {
    ...
    while( ok && startAddr <= endAddr )
    {
      ...
      if(  //< check enumeration filter, if provided
        ok &&
        onEnum && 
        !onEnum(
          io, 
...
static inline esBL ekosfStdSlaveIsAddressed(const EseStdIoState* client)
{
  ...
  if( client->hdr.dataLen == 6/*sizeof(esU32)+2*/ )
  {
    ...
    if( chnlIsRateSupported(client->chnl, rate) )
    {
      ekosfUpdateHdr(
        &client->hdr, 
        ES_FT_SLAVE, 
        ES_MAKE_RESPONSE(ES_CMD_RATE_SET), 
        0
      );
      
      ekosfSetPendingRateChange(
        client, 
        rate
      );
    }
...
static inline esU32 pseudoRandomTimeSliceGet(void)
{
  esU32 clk = clock();
...
static void ekosfStdSlavePendingUpdatesHandle(EseStdIoState* client)
{
  ...
  if( 
    0 != client->pendingRate || 
    -1 != client->pendingAddr 
  )
  {
    if( chnlWaitTxEmpty(client->chnl) )
    {
      if( 0 != client->pendingRate ) //< execute rate change
        chnlSetRate(
          client->chnl, 
          client->pendingRate
        );

        if( -1 < client->pendingAddr ) //< update pending address change
...
esBL ekosfStdSlaveExecute( EseStdIoState* client )
{
  ...
  if( 
    !ekosfGetCheckHdr(
      client, 
      ekosfHdrFrameReceiveMaxRetries, 
      TRUE
    ) 
  )
    return FALSE;
