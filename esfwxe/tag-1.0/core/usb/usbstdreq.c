...
static BYTE bConfiguration = 0;                       /** Currently selected configuration */
static TFnHandleRequest *pfnHandleCustomReq = NULL; 	/** Installed custom request handler */
static const BYTE *pabDescrip = NULL;                 /** Pointer to registered descriptors */
...
//
BOOL usbGetDescriptor (WORD wTypeIndex, WORD wLangID __attribute__ ((unused)), int *piLen, BYTE **ppbData)
{
  BYTE  bType, bIndex;
  BYTE  *pab;
  int iCurIndex;
...
//
static BOOL usbSetConfiguration (BYTE bConfigIndex, BYTE bAltSetting)
{
  BYTE  *pab;
  BYTE  bCurConfig, bCurAltSetting;
  BYTE  bEP;
  WORD wMaxPktSize;
...
//
static BOOL usbHandleStdDeviceReq (TSetupPacket *pSetup, int *piLen, BYTE **ppbData)
{
  BYTE  *pbData = *ppbData;
...
//
static BOOL usbHandleStdInterfaceReq (TSetupPacket  *pSetup, int *piLen, BYTE **ppbData)
{
  BYTE  *pbData = *ppbData;
...
// 
static BOOL usbHandleStdEndPointReq (TSetupPacket *pSetup, int *piLen, BYTE **ppbData)
{
  BYTE  *pbData = *ppbData;
