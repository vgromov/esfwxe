...
#ifndef NDEBUG
#	include <assert.h>
#	define BGET_ASSERT		assert
#else
# define BGET_ASSERT(x)
#endif
...
{	bufsize curalloc;	
	...
	TRACE5(	"memory statistics:\n\r"											 
					"\tallocated|free: %u|%u\n\r"										 
					"\tlargest free block: %u\n\r"									 
					"\tmalloc|free calls: %d|%d\n\r", 							 
...
void brel(void *buf)
{
	...
	BGET_ASSERT(b->bh.prevfree == 0);
	BGET_ASSERT(BH((char *) b + b->bh.bsize)->bsize == ESent);
	BGET_ASSERT(BH((char *) b + b->bh.bsize)->prevfree == b->bh.bsize);
	...
	BGET_ASSERT(numpblk == numpget - numprel);
#endif /* BufStats */
    }
#endif /* BECtl */
}
...
int main()
{
	...
	BGET_ASSERT(bs <= (((bufsize) 4) * ExpIncr));
	bs = blimit(bs);
	if (rand() & 0x400) {
	    cb = (char *) bgetz(bs);
	} else {
