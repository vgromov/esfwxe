...
enum {
  ...
  sht21ReadTFromRh    = 0xE0, // read T from previous RH measurement in normal I2C mode (no hold master)
...
// sht21 api
//
esBL sht21SoftResetStart(i2cHANDLE h)
{
  esU8 tmp = sht21Softreset;
  return 1 == i2cPutBytes(h, sht21Addr, &tmp, 1, true);
}

esBL sht21SoftReset(i2cHANDLE h)
{
  esU8 tmp = sht21Softreset;
  if( 1 == i2cPutBytes(h, sht21Addr, &tmp, 1, true) )
  {
    // wait for sht21 chip to reset
    msDelay(sht21softResetWait);
    return TRUE;
  }
...
esBL sht21UserRegisterRead(i2cHANDLE h, esU8* ur)
{
  ...
  return   1 == i2cPutBytes(h, sht21Addr, &tmp, 1, false) &&
...
esBL sht21UserRegisterWrite(i2cHANDLE h, esU8 ur)
{
  ...
  tmp[1] = ur;
  return 2 == i2cPutBytes(h, sht21Addr, tmp, 2, true);  
}
...
esBL sht21TemperatureStart(i2cHANDLE h, esU8 precision)
{
  ...
  if( sht21UserRegisterRead(h, &ur) )
  {
    ...
    return sht21UserRegisterWrite(h, ur) &&
      1 == i2cPutBytes(h, sht21Addr, &cmd, 1, true);
  }
...
esBL sht21TemperatureReadFromRh(i2cHANDLE h, long* result)
{
  esU8 tmp[2];
  
  tmp[0] = sht21ReadTFromRh;
  if( 1 == i2cPutBytes(h, sht21Addr, tmp, 1, false) &&
    2 == i2cGetBytes(h, sht21Addr, tmp, 2)
  )
  {
    long raw;
    // clear status bits from LSB 
...
esBL sht21TemperatureGetBlocking(i2cHANDLE h, esU8 precision, long* result)
{
  ...
  {
    msDelay( c_sht21waitT[precision] );
...
esBL sht21HumidityGetBlocking(i2cHANDLE h, esU8 precision, long* result)
{
  ...
  {
    msDelay( c_sht21waitRh[precision] );
    return sht21ResultGet(h, result, 0);
  }
